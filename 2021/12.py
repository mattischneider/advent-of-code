class Cave:
    ...
    def add_neighbour_cave(self, neighbour_cave: "Cave"):
        if neighbour_cave not in self.neighbours:
...
def get_paths(
    ...
    max_small_visits_allowed: int = 1,
) -> list[list[Cave]]:
    ...
    while current_search_paths:
        ...
        for nc in current_cave.neighbours:
            ...
            other_small_visits = max(
                sum(1 for x in current_path if x == y)
                for y in current_path
                if y.is_small and y != nc
            )
            if nc.is_small and times_nc_is_visited == max_small_visits_allowed:
                continue
            if (
                max_small_visits_allowed > 1
                and nc.is_small
                and times_nc_is_visited == max_small_visits_allowed - 1
                and other_small_visits == max_small_visits_allowed
            ):
                continue
