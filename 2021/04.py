class BingoBoard:
    def __init__(self, raw_board: str):
        ...
        self.board = [[int(i) for i in j.split(" ") if i != ""] for j in _splitted if j != ""]
        self.size = len(self.board)
        self.drawn_numbers = [[0 for _ in range(self.size)] for _ in range(self.size)]
    ...
    def is_solved(self):
        ...
        vert = [sum(i[j] for i in self.drawn_numbers) for j in range(self.size)]
        return any(j == self.size for j in hor) or any(j == self.size for j in vert)
