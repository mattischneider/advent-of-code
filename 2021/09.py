...
class HeightMap:
    ...
    def low_points(self) -> list[LocationPoint]:
        ...
        for p in self.all_location_points:
            neighbours = [
                self.get_location_point_by_position(x, y) for x, y in p.get_neighbour_positions()
            ]
    ...
    def get_basin_size(self, low_point: LocationPoint) -> int:
        ...
        points_to_explore: list[LocationPoint] = [
            self.get_location_point_by_position(x, y)
            for x, y in low_point.get_neighbour_positions()
        ]
        ...
        while points_to_explore:
            ...
            if next_point.height < 9:
                ...
                for neighbour in [
                    self.get_location_point_by_position(x, y)
                    for x, y in next_point.get_neighbour_positions()
                ]:
...
if __name__ == "__main__":
    ...
    basin_sizes = [map.get_basin_size(line) for line in map.low_points]
    basin_sizes.sort()
