def get_lowest_risk_path(cave: list[str]) -> int:
    risk_levels = [[{'node_risk': int(c), 'path_risk': None}
                    for c in t.strip()] for t in cave]
    ...
    while not target_node in reachabled_nodes:
        ...
        for idx, idy in neighbours:
            ...
            risk_to_neighbour_from_current = risk_levels[neighbour_x][neighbour_y]['node_risk'] + \
                current_min_path_risk
            ...
            if curren_neighbour_path_risk is None or \
                    curren_neighbour_path_risk > risk_to_neighbour_from_current:
...
def extend_cave(cave: list[str], times=5) -> list[str]:
    ...
    new_cave = [[float('inf') for _ in range(times*cave_size)]
                for _ in range(times*cave_size)]
    ...
    for idx in range(times*cave_size):
        for idy in range(times*cave_size):
            if idx < cave_size and idy < cave_size:  # original cave
                ...
                continue
            extended_node_risk = cave_risks[idx % cave_size][idy % cave_size] + \
                idx//cave_size + idy//cave_size
