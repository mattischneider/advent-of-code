...
for i in range(n):
    ...
    if oxy_number_of_bits_set >= oyx_n / 2:
        possible_oxygen_generators = [p for p in possible_oxygen_generators if p[i] == "1"]
    else:
        possible_oxygen_generators = [p for p in possible_oxygen_generators if p[i] == "0"]
    ...
    if co_number_of_bits_set >= co_n / 2:
        possible_co_scrubbers = [p for p in possible_co_scrubbers if p[i] == "0"]
    else:
        possible_co_scrubbers = [p for p in possible_co_scrubbers if p[i] == "1"]
