...
class TileFloor:
    ...
    def flip(self, instruction):
        instruction_steps = [i for i in re.split(r"(ne|se|sw|nw|w|e)", instruction) if i != ""]
        instruction_steps = [
            i.replace("ne", "1, 2")
            .replace("se", "-1, 2")
            .replace("sw", "-1, -2")
        ...
        instruction_steps = [list(map(int, i.split(", "))) for i in instruction_steps]
        position = [sum(i[0] for i in instruction_steps), sum(i[1] for i in instruction_steps)]
