        ...
        for idy in ranges:
            if (idx, idy) != (0, 0):
                for current_looking_distance in range(1, looking_distance + 1):
                    look_at_i = i + idx * current_looking_distance
                    look_at_j = j + idy * current_looking_distance
...
def update_seatingplan_position(i, j, seatingplan, max_treshhold, looking_distance):
    ...
    num_of_vis_seats = get_number_of_visible_seats(i, j, seatingplan, looking_distance)
...
def passengers_finding_seats(seatingplan, max_treshhold, looking_distance):
    ...
    while more_steps_needed is True:
        step_number += 1
        new_seatingplan = copy.deepcopy(seatingplan)
...
# part 1:
print(passengers_finding_seats(SEATINGPLAN_START, max_treshhold=4, looking_distance=1))

# part 2:
print(
    passengers_finding_seats(
        SEATINGPLAN_START, max_treshhold=5, looking_distance=len(SEATINGPLAN_START)
    )
)
