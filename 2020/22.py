class Combat:
    ...
    def get_winner_and_winning_score(self):
        ...
        if len(self.player_2_cards) == 0:
            winner = 1
        winning_score = sum((len(final_deck) - idx) * card for idx, card in enumerate(final_deck))
    ...
    def next_recursive_round(self):
        if [
            self.player_1_cards,
            self.player_2_cards,
        ] in self.game_history:  # to avoid infinite games
            game_winner = 1
            winning_score = 0  # score does not matter in sub-games
            self.player_2_cards = []
            return game_winner, winning_score
        else:
            self.game_history.append([list(self.player_1_cards), list(self.player_2_cards)])
        p1_top_card = self.player_1_cards.pop(0)
        p2_top_card = self.player_2_cards.pop(0)
