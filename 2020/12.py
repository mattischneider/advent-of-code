FILE_NAME = "2020/12.txt"

with open(FILE_NAME, "r") as f:
    INSTRUCTIONS = f.read().split("\n")
    INSTRUCTIONS = [{"action": i[0], "value": int(i[1:])} for i in INSTRUCTIONS]
...
for i in INSTRUCTIONS:
    ...
    if i["action"] == "W":
        WAYPOINT_POSITION[0] -= i["value"]
    if i["action"] in ("L", "R"):
        WAYPOINT_POSITION = get_new_waypoint_position(WAYPOINT_POSITION, i["value"], i["action"])
    if i["action"] == "F":
