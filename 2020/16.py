...
def is_in_restriction_range(restriction, ticket_value):
    in_any_range = any(
        [
            is_ticket_value_in_range(ticket_value=ticket_value, bounds_range=restriction[cond])
            for cond in restriction
        ]
    )
    return in_any_range
...
def get_invalid_ticket_value(ticket, restrictions):
    for ticket_value in ticket:
        if not any(
            [
                is_in_restriction_range(restriction=restrictions[r], ticket_value=ticket_value)
                for r in restrictions
            ]
        ):
...
def get_position_of_restriction_name(restriction, all_valid_tickets):
    ...
    for i in range(0, len(all_valid_tickets[0])):
        ...
        if all(
            [
                is_in_restriction_range(restriction=restriction, ticket_value=ticket_value)
                for ticket_value in ticket_values_at_position_i
            ]
        ):
...
invalid_ticket_values = [get_invalid_ticket_value(nt, restrictions) for nt in nearby_tickets]
print(
    sum(i["invalid_ticket_value"] for i in invalid_ticket_values if i["is_valid_ticket"] is False)
)
...
valid_tickets = [
    ...
    if get_invalid_ticket_value(t, restrictions)["is_valid_ticket"] is True
]
...
while True:
    for r in restrictions:
        ...
        if len(valid_positions) == 1:
            ...
            continue
    res_pos = [len(restrictions[r]["valid_positions"]) == 1 for r in restrictions]
    if all(res_pos):
        break
