...
class ConwayCube:
    ...
    def get_next_spreading(self, dimension):
        current_spread = [active_cube[dimension] for active_cube in self.active_cubes]
    ...
    def get_all_neighbours(self, v):
        ...
        for i in range(0, 3 ** len(v)):
            ...
            for j in range(self.number_of_dimensions - len(nums)):
                nums = [-1] + nums
            a = [nums[idx] + v[idx] for idx in range(self.number_of_dimensions)]
            neighbours.append(a)
        neighbours.remove(v)
        return neighbours
    ...
    def get_new_status(self, v):
        ...
        number_of_active_neighbour_cubes = sum(1 for n in neighbours if n in self.active_cubes)
