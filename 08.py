...
def get_decode_dict(signals: list[str]) -> dict[str, str]:
    for s in signals:
        if len(s) == 2:
            cf = set(char for char in s)
        if len(s) == 3:
            acf = set(char for char in s)
        if len(s) == 4:
            bcdf = set(char for char in s)
        if len(s) == 7:
            abcdefg = set(char for char in s)
    a = acf - cf
...
if __name__ == '__main__':
    ...
    print('part1:', sum(sum(1 for a in x[1] if len(
        a) in [2, 3, 4, 7]) for x in all_signals))

    final = 0
    for s in all_signals:
        decode_dict = get_decode_dict(s[0]+s[1])
        final += int(''.join(decode_value(output, decode_dict)
                     for output in s[1]))
    print('part2:', final)
